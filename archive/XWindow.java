...
//        insLog.finer("Configure, {0}, event disabled: {1}",xev.get_xconfigure(), isEventDisabled(xev));
        if (isEventDisabled(xev)) {
            return;
        }
...
//        log.fine("Mapped {0}", this);
    ...
    private void dumpKeysymArray(XKeyEvent ev) {
        System.out.println("  "+Long.toHexString(XlibWrapper.XKeycodeToKeysym(XToolkit.getDisplay(), ev.get_keycode(), 0))+
    ...
    void logIncomingKeyEvent(XKeyEvent ev) {
        System.out.println("--XWindow.java:handleKeyEvent:" + ev);
        dumpKeysymArray(ev);
        System.out.println("XXXXXXXXXXXXXX javakeycode will be most probably:0x" + Integer.toHexString(XKeysym.getJavaKeycodeOnly(ev)));
    }
    public void handleKeyPress(XEvent xev) {
        super.handleKeyPress(xev);
        XKeyEvent ev = xev.get_xkey();
//        if (eventLog.isLoggable(PlatformLogger.FIN)) 
...
//                if (keyEventLog.isLoggable(PlatformLogger.FINE)) {
                System.out.println("--XWindow.java XIM did process event; return; dec keysym processed:"+(keysym[0])+
...
//                if (keyEventLog.isLoggable(PlatformLogger.FINE)) {
                System.out.println("--XWindow.java XIM did NOT process event, hex keysym:"+Long.toHexString(keysym[0])+"\n"+
...
//                }
            ...
            unicodeKey = keysymToUnicode( keysym[0], ev.get_state() );
//            if (keyEventLog.isLoggable(PlatformLogger.FINE)) {
                System.out.println("--XWindow.java XIM is absent;             hex keysym:"+Long.toHexString(keysym[0])+"\n"+
                        "                                         unicode key:"+Integer.toHexString((int)unicodeKey));
//            }
        }
...
//        }
        ...
        int jkeyExtended = jkc.getJavaKeycode() == java.awt.event.KeyEvent.VK_UNDEFINED ?
                ...
                jkc.getJavaKeycode();
        System.out.println("!!!!!!!!!!!!!");
        System.out.println("unicodeKey="+unicodeKey+" jkeyToReturn="+jkeyToReturn+" ev.get_state()="+ev.get_state()+" ev.getPData()="+ev.getPData()+" (long)(ev.get_keycode())="+(long)(ev.get_keycode())+" unicodeFromPrimaryKeysym="+unicodeFromPrimaryKeysym+" jkeyExtended="+jkeyExtended );
        ...
        if( unicodeKey > 0 ) {
            System.out.println("fire _TYPED on "+unicodeKey);
